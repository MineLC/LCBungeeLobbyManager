package lc.cbungeelobby;

import lc.cbungeelobby.commands.*;
import lc.cbungeelobby.configuration.ConfigManager;
import lc.cbungeelobby.entities.Type;
import lc.cbungeelobby.listeners.Event;
...
public final class LCBungeeLobby extends Plugin {
    ...
    public void loadDeniedServers(){
        ...
        for(String servers : lobbys){
            if(getProxy().getServers().containsKey(servers)){
                Rlobbys.add(servers);
            }
        }
    }
    ...
    public static LCBungeeLobby getInstance(){
        return instance;
    }
